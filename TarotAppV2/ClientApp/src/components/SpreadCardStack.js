...
const SpreadCardStack = ({ pickedCards }) => {
    ...
    const handleCardClick = (cardIndex) => {
        ...
        pickedCards(selectedCards.length);
    };
        ...
        <div className="backOfCard-container">
            {[...Array(cardCount)].map((_, index) => (
                ...
                >
                    <div
                        key={index+1}
                        className={`backOfCard backOfCard-${index + 1} ${selectedCards.includes(index+1) ? 'moved' : ''}`}
                        onClick={() => handleCardClick(index)}
                    >
                        <div className="backOfCard-body">
